        ...
        const handleSavePicture=async ()=>{
          ...
          if(!err){
            var body = {"categoryname":categoryName,"categoryid":categoryId}
              var result = await postData("categories/updatecategories",body);
              if(result)
              {
                Swal.fire({
                  title: 'GlassKart.com',
          ...
          const handleDeleteCategory = async (data) => {
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
                 result = await postData("categories/deletecategories", body);
                if (result){
      ...
      <MaterialTable
        title={
          <>
            ...
            <b style={{ fontSize: 18, opacity: 0.5 }}> CATEGORIES </b>{" "}
              <Tooltip title="Add Category" >
                <Fab color="primary" size="small" onClick={() => props.setComponent(<AddCategories />)} className={classes.fab}>
                  <AddIcon />
                </Fab>
              </Tooltip>
          </>
