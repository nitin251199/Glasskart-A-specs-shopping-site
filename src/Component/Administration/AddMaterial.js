  ...
  export default function AddMaterial(props) {
    ...
    const handlePicture = (event)=>{
      ...
      if(isEmpty(materialName)){
        err = true;
        errorMessage("Material Name should not be empty");
      ...
      if(isEmpty(adPicture.filename)){
        err = true;
        errorMessage("Please Add Material Picture..."); 
      }
      if(!err){
      ...
      return(
       ...
       <Grid container xs={12} spacing={1} style={{display: 'flex',justifyContent: 'center',alignItems: 'center'}}>
        ...
        <Grid item xs={6}>
          <TextField variant="outlined" fullWidth label="Material Name"
      ...
      <label htmlFor="contained-button-file">
        ...
        <Avatar alt="Remy Sharp" src={adPicture.filename} variant="rounded" className={classes.large} />
         ...
         <Grid item md={12}>
      <Button variant="contained" color="primary" style={{background:"#22a6b3" }} fullWidth onClick={()=>handleSubmit()} >Submit Material</Button>
      </Grid>
       </Grid>
        </div>
      </div>
