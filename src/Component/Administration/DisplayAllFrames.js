...
const fetchAllFrames = async()=>{
    var list = await getData('frame/fetchallframes')
  ...
  const handleCancelPicture=async ()=>{
    ...
    const handleSavePicture=async ()=>{
        ...
        var config = { headers: { "content-type": "multipart/form-data" } };
          var result = await postDataAndImage("frame/editadpicture", formData, config);
          if(result)
          {
            Swal.fire({
              title: 'GlassKart.com',
      ...
      if(isEmpty(adPicture.filename)){
        ...
        if(!err){
          var body = {"framename":frameName,"status":status,"frameid":frameid}
            var result = await postData("frame/updateframe",body);
            if(result)
          {
            Swal.fire({
              title: 'GlassKart.com',
      ...
      const handleDeleteFrame = async (data) => {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
             result = await postData("frame/deleteframe", body);
            if (result){
  ...
  <MaterialTable
    title={
      <>
        ...
        <b style={{ fontSize: 18, opacity: 0.5 }}> FRAME TYPES </b>{" "}
          <Tooltip title="Add Frame Types" >
            <Fab color="primary" size="small" onClick={() => props.setComponent(<AddFrameTypes />)} className={classes.fab}>
              <AddIcon />
            </Fab>
          </Tooltip>
      </>
