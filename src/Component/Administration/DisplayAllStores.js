...
if(!err){
  var body = {"storeid":storeId,"state":state,"city":city,"storename":storeName,"addressone":addressOne,
  "addresstwo":addressTwo,"landmark":landmark,"latitude":latitude,"longitude":longitude,"contactno":contactNo,
    ...
    const storeDialog=()=>{
      return (
       ...
       <Grid container xs={12} spacing={1} style={{display: 'flex',justifyContent: 'center',alignItems: 'center'}}>
        ...
        <Grid item xs={6}>
          <TextField variant="outlined" value={city} fullWidth label="City"
          onChange={(event)=>setCity(event.target.value)}
          />
        </Grid>

        <Grid item xs={12}>
          <TextField variant="outlined" value={storeName} fullWidth label="Store Name"
          onChange={(event)=>setStoreName(event.target.value)}
          />
        </Grid>

        <Grid item xs={12}>
          <TextField variant="outlined" fullWidth value={addressOne} label="Address One"
          onChange={(event)=>setAddressOne(event.target.value)}
          />
        </Grid>
        
        <Grid item xs={12}>
          <TextField variant="outlined" fullWidth value={addressTwo} label="Address Two"
          onChange={(event)=>setAddressTwo(event.target.value)}
          />
        </Grid>

        <Grid item xs={12}>
          <TextField variant="outlined" fullWidth value={landmark} label="Landmark"
        ...
        <Grid item xs={6}>
          <TextField variant="outlined" value={contactNo} fullWidth label="Contact Number"
          onChange={(event)=>setContactNo(event.target.value)}
          />
        </Grid>

        <Grid item xs={6}>
          <TextField variant="outlined" fullWidth value={emailAddress} label="Email address"
      ...
      <Button onClick={()=>handleCancelPicture()}>Cancel</Button></>:<></>}
        ...
        <Avatar alt="" src={picture.filename} variant="rounded" className={classes.large} />
         ...
         <Grid item md={12}>
      <Button variant="contained" color="primary" style={{background:"#22a6b3" }} fullWidth onClick={()=>handleSubmit()} >Edit Store</Button>
      </Grid>
       </Grid>
        </div>
      </div>
        ...
        return (
          <MaterialTable
              ...
              <>
                ...
                <b style={{ fontSize: 18, opacity: 0.5 }}> OUR STORES </b>{" "}
                  <Tooltip title="Add Stores" >
                    <Fab color="primary" size="small" onClick={() => props.setComponent(<AddStoreCity />)} className={classes.fab}>
                      <AddIcon />
                    </Fab>
                  </Tooltip>
              </>
            ...
            columns={[
              ...
              { title: 'Location', 
               render: rowData =><div><a href={`https://maps.google.com/?q=${rowData.lat},${rowData.lng}`}>Show</a></div> },
              { title: 'Contact',
               render: rowData =><div>{rowData.contactno}<br/>{rowData.emailaddress}</div>},
               {
                title: "Picture",
                render: (rowData) => (
                  <img
