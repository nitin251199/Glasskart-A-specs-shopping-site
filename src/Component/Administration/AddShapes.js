  ...
  export default function AddShape(props) {
    ...
    const handlePicture = (event)=>{
      ...
      if(isEmpty(adPicture.filename)){
        err = true;
        errorMessage("Please Add Shape Picture..."); 
      }
      if(!err){
        ...
        var config = { headers: { "content-type": "multipart/form-data" } };
          var result = await postDataAndImage("shapes/insertshape", formData, config);
          if(result)
          {
            Swal.fire({
              title: 'GlassKart.com',
      ...
      return(
       ...
       <Grid container xs={12} spacing={1} style={{display: 'flex',justifyContent: 'center',alignItems: 'center'}}>
        ...
        <Grid item xs={6}>
          <TextField variant="outlined" fullWidth label="Shape Name"
      ...
      <label htmlFor="contained-button-file">
        ...
        <Avatar alt="Remy Sharp" src={adPicture.filename} variant="rounded" className={classes.large} />
         ...
         <Grid item md={12}>
      <Button variant="contained" color="primary" style={{background:"#22a6b3" }} fullWidth onClick={()=>handleSubmit()} >Submit Shape</Button>
      </Grid>
       </Grid>
        </div>
      </div>
