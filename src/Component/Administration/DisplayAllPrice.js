    ...
    const handleDeleteOpen =async (data) => {
      ...
      Swal.fire({
       ...
       if (result.isConfirmed) {
        result=await postData('price/deleteprice',body)
        if(result)
         Swal.fire(
           'Deleted!',
        ...
        <DialogTitle id="form-dialog-title">
          <div style={{display:"flex",justifyContent:"center",alignItems:"center",flexDirection:"column",padding:5}}>
            <div style={{fontSize:20,fontWeight:"bold",letterSpacing:1,padding:5}}>
              <span><img alt="" src='/glasskart.png' width='40'/></span>
      ...
      <MaterialTable
        title={
          <>
            ...
            <b style={{ fontSize: 18, opacity: 0.5 }}> PRICE RANGE </b>{" "}
              <Tooltip title="Add Price Range" >
                <Fab color="primary" size="small" onClick={() => props.setComponent(<AddPrice />)} className={classes.fab}>
                  <AddIcon />
                </Fab>
              </Tooltip>
          </>
