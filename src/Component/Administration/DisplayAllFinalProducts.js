...
import AddFinalProducts from "./AddFinalProducts";
...
const useStyles = makeStyles((theme) => ({
  ...
  export default function DisplayAllFinalProducts(props){
    ...
    DisplayAllFinalProducts.modules = {
      toolbar: [
     ...
     */
    DisplayAllFinalProducts.formats = [
      'header', 'font', 'size',
      'bold', 'italic', 'underline', 'strike', 'blockquote',
      'list', 'bullet', 'indent',
      'link', 'image', 'video'
    ...
    const fetchAllProducts = async()=>{
      ...
      const handleCancelPicture=async ()=>{
        ...
        const handleSavePicture=async ()=>{
          ...
          const handleSubmit = async () =>{
            ...
            if(!err){
              var body = {"finalproductid":finalProductId,"productid":productId,"colorid":colorId,"size":size,"price":price,
              "offertype":offerType,"offerprice":offerPrice,"description":description,"stock":stock}
            ...
            const handleClickOpen = (data) => {
              ...
              setStock(data.stock)
              setProductPicture({ filename: `${ServerURL}/images/${data.productpicture}`, bytes: "" })
              setOpen(true);
            };
        ...
        return (
         ...
         <Grid item xs={6}>
          ...
          <Grid item xs={6}>
            ...
            <Grid item xs={12}>
              <TextField variant="outlined" value={price} fullWidth label="Price"
              onChange={(event)=>setPrice(event.target.value)}
              />
            </Grid>

            <Grid item xs={12}>
              <TextField variant="outlined" fullWidth value={offerPrice} label="Offer Price"
            ...
            <ReactQuill
            value={description} 
            modules={DisplayAllFinalProducts.modules}
            formats={DisplayAllFinalProducts.formats}
            onChange={(txt)=>setDescription(txt)} />
             </Grid>

            <Grid item xs={12}>
              <TextField variant="outlined" fullWidth value={stock} label="Stock"
      ...
      <Button onClick={()=>handleCancelPicture()}>Cancel</Button></>:<></>}
            ...
            <Avatar alt="" src={productPicture.filename} variant="rounded" className={classes.large} />
             ...
             <Grid item md={12}>
          <Button variant="contained" color="primary" style={{background:"#22a6b3" }} fullWidth onClick={()=>handleSubmit()} >Edit Final Product</Button>
          </Grid>
           </Grid>
          </div>
        </div>
                ...
                <MaterialTable
                  title={
                    <>
                      ...
                      <b style={{ fontSize: 18, opacity: 0.5 }}> FINAL PRODUCTS </b>{" "}
                        <Tooltip title="Add Final Products" >
                          <Fab color="primary" size="small" onClick={() => props.setComponent(<AddFinalProducts />)} className={classes.fab}>
                            <AddIcon />
                          </Fab>
                        </Tooltip>
                    </>
                  ...
                  columns={[
                    ...
                    { title: 'Product Name',
                    render: rowData =><div><b>{rowData.productname}</b></div>},
                    { title: 'Color', 
                     render: rowData =><div>{rowData.colorname}</div>},
                    { title: 'Size', 
                     render: rowData =><div>{rowData.size}</div> },
                    { title: 'Price',
                     render: rowData =><div>{rowData.price}</div>},
                     { title: 'Offer',
                     render: rowData =><div>{rowData.offerprice}<br/>{rowData.offertype}</div>},
                     { title: 'Stock',
