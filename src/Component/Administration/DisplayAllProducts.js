      ...
      const handleCancelPicture=async ()=>{
        ...
        const handlePicture = (event)=>{
          ...
          const handleSubmit = async () =>{
            ...
            if(isEmpty(adStatus)){
              err = true;
              errorMessage("Ad Status should not be empty"); 
            }
          if(isEmpty(picture.filename)){
            ...
            if(!err){
                var body = {"productid":productId,"productname":productName,"type":type,"categoryid":categoryId
                ,"frameid":frameId,"materialid":materialId,"shapeid":shapeId, "description":description,
                  "status":status,"adstatus":adStatus}
     ...
     const handleClickOpen = (data) => {
      ...
      const storeDialog=()=>{
        return (
         ...
         <Grid item xs={6}>
            ...
            <Grid item xs={6}>
              <TextField variant="outlined" value={productName} fullWidth label="Product Name"
              onChange={(event)=>setProductName(event.target.value)}
              />
            </Grid>

            <Grid item xs={6}>
              <TextField variant="outlined" value={status} fullWidth label="Status"
            ...
            <Grid item xs={6}>
            <FormControl variant="outlined" fullWidth>
            <InputLabel id="adstatus-id">Select AdStatus</InputLabel>
            <Select
              labelId="adstatus-id"
              id="adstatusid"
              value={adStatus}
              onChange={(event)=>setAdStatus(event.target.value)}
              label="Select AdStatus"
            >
      ...
      <Button onClick={()=>handleCancelPicture()}>Cancel</Button></>:<></>}
            ...
            <Avatar alt="" src={picture.filename} variant="rounded" className={classes.large} />
             ...
             <Grid item md={12}>
          <Button variant="contained" color="primary" style={{background:"#22a6b3" }} fullWidth onClick={()=>handleSubmit()} >Update Product</Button>
          </Grid>
           </Grid>
          ...
          <MaterialTable
            title={
              <>
                ...
                <b style={{ fontSize: 18, opacity: 0.5 }}> PRODUCTS </b>{" "}
                  <Tooltip title="Add Products" >
                    <Fab color="primary" size="small" onClick={() => props.setComponent(<AddProducts />)} className={classes.fab}>
                      <AddIcon />
                    </Fab>
                  </Tooltip>
              </>
            ...
            columns={[
              ...
              { title: 'Type', 
               render: rowData =><div>{rowData.type}</div>},
              { title: 'Category', 
               render: rowData =><div>{rowData.categoryname}</div> },
              { title: 'Frame',
               render: rowData =><div>{rowData.framename}</div>},
               { title: 'Material',
               ...
               { title: 'AdStatus',
               render: rowData =><div>{rowData.adstatus}</div>},
               {
                title: "Picture",
                render: (rowData) => (
                  <img
